...
const uint32_t RoutingProtocol::AODV_PORT = 954;//654;
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
	...
	// hostsTable for the general hosts table
	.AddAttribute ("IbrHostsTable", "IBR HostsTable for general information of node location",
					PointerValue(),
					MakePointerAccessor(&RoutingProtocol::SetHostsTable),
					MakePointerChecker<IbrHostsTable>()				
				   )
...
void RoutingProtocol::SetHostsTable(Ptr<IbrHostsTable> iHT) {
	  NS_LOG_FUNCTION(iHT);
	  m_IbrHostsTable=iHT;
	}
...
RoutingProtocol::Start ()
{
  ...
  // for print routing tables
  std::ostringstream temp;
  uint32_t nodeId=m_ipv4->GetObject<Node>()->GetId();
  if (nodeId>540 && nodeId<560) {
   temp << "vnaodv-routing-tables_" << m_ipv4->GetObject<Node>()->GetId() << ".txt";
...
{
  //NS_LOG_DEBUG("*** RouteOutput Looking for route  to " << header.GetDestination() );
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  //NS_LOG_DEBUG("** RouteInput Looking for route from " << idev->GetAddress() << " to " << header.GetDestination() );
...
RoutingProtocol::SendTo (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
	...
	RoutingProtocol::RecordLog(0, 1, 32);
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  //NS_LOG_UNCOND(" *** sender: " << sender << "\treceiver: " << receiver);
...
/*
  // conexion con vnlayer
  // receiver -> es la ip del nodo lider que busca el nodo movil en su region
  // virtualIp -> es la ip virtual del nodo lider actual
  // destIp -> es la ip de destino del paquete (obtenida del paquete routeRequest)
  

  
  Ptr<Packet> pkt1 = packet->Copy();
  RreqHeader rreqHeader;
  pkt1->RemoveHeader (rreqHeader);
	


  //NS_LOG_WARN("DST: " << rreqHeader.GetDst());
  //if (rreqHeader.GetDst()==Ipv4Address("10.0.0.119") ) {
  //if (receiver==Ipv4Address("10.0.2.42") || sender == Ipv4Address("10.0.2.42")) {
  	// verificamos si el nodo se encuentra el la region vnLayer
  	//NS_LOG_DEBUG("Buscando IP en nodo "  );
	
	Ipv4Address virtualIp=m_ipv4->GetAddress(2,0).GetLocal();
	Ipv4Address localIp=m_ipv4->GetAddress(1,0).GetLocal();
	//Ipv4Address destIp=rreqHeader.GetDst();
  	//NS_LOG_WARN(" *buscando IP: " << rreqHeader.GetDst()  <<  " en " << receiver << " en lider " << sender << "[" << virtualIp << "]" );

  	if (RoutingProtocol::isNodeOnVnRegion(virtualIp , rreqHeader.GetDst())) {
 	 	NS_LOG_WARN(" ****** nodo " << rreqHeader.GetDst() << " encontrado en el lider " << sender << "[" << virtualIp << "]" << "[" << localIp << "] - Receiver: " << receiver );
  	}
  //}
*/
  if (!tHeader.IsValid ())
  ...
  switch (tHeader.Get ())
	...
	RoutingProtocol::RecordLog(1, 1, 32);
}
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      m_routingTable.LookupRoute (origin, toOrigin);
      NS_LOG_WARN ("Send reply since I am the destination");
      SendReply (rreqHeader, toOrigin);
      return;
    }

  //  pato
  //  (ii) the requested node is in the vnLayer Region
  Ipv4Address virtualIp=m_ipv4->GetAddress(2,0).GetLocal();
  //Ipv4Address localIp=m_ipv4->GetAddress(1,0).GetLocal();
  Ipv4Address destIp=rreqHeader.GetDst();
  if (RoutingProtocol::isNodeOnVnRegion(virtualIp , rreqHeader.GetDst())) {
 	 	NS_LOG_WARN(" ****** Nodo (" <<  m_ipv4->GetObject<Node> ()->GetId ()
<< ") " << destIp << " encontrado en el lider " << "[" << virtualIp << "]" << " - Receiver: " << receiver );
  		Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
	...
	RoutingTableEntry newEntry (	/*device=*/ dev, 
								...
								/*validSeno=*/ true, 
									/*seqNo=*/ rreqHeader.GetOriginSeqno (),
                               		/*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (virtualIp), 0), 
									/*hops=*/ hop,
                                   /*nextHop*/ destIp, 
									/*timeLife=*/ Time(15) //Time ((2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime))
								);
      m_routingTable.AddRoute (newEntry);
	  
//	  hop++;	
		
  	 RoutingTableEntry newEntry1 (	/*device=*/ dev, 
					  				/*dst=*/ virtualIp, 
									/*validSeno=*/ true, 
									/*seqNo=*/ rreqHeader.GetOriginSeqno (),
                             		/*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0), 
     ...
     m_routingTable.AddRoute (newEntry1);
	...
	m_routingTable.LookupRoute (origin, toOrigin);
	  ...
	  NS_LOG_WARN ("Send reply since I am the destination leader >> " << origin << " to " << virtualIp << ":" << destIp );
      SendReply (rreqHeader, toOrigin);
	  		// reenviamos el paquete al nodo solicitado por unicast 
...
bool RoutingProtocol::isNodeOnVnRegion(Ipv4Address leader, Ipv4Address dst) {
	return RoutingProtocol::m_IbrHostsTable->isNodeInRegion(leader, dst);

}

void RoutingProtocol::RecordLog(uint8_t TxRx, uint32_t pkts, uint32_t byteSize){
		std::string dir;
		...
		NS_LOG_DEBUG( " Time:" << Simulator::Now().GetSeconds() << "\t" << "Pkt:VNAodv-" << dir << "\t" << "Msg:" << "AODVPkt" << "\t" <<  "Pkts:" << pkts << "\t" << "Size:" << byteSize );
