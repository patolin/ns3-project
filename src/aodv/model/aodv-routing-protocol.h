...
public:
  ...
  */
  ...
  // virtual layer host table
  // to use with vnlayer
  Ptr<IbrHostsTable> m_IbrHostsTable;
  void SetHostsTable(Ptr<IbrHostsTable> iHT);
  bool isNodeOnVnRegion(Ipv4Address leader, Ipv4Address dst);
...
private:
  ...
  // record log
  void RecordLog(uint8_t TxRx, uint32_t pkts, uint32_t byteSize);
